...
file_path = 'C:/Users/Usuario/GASTON/Z Sintesis de Redes Activas/Ejercicios (Vs Code)/.venv(enviroment)/Lab2_1b_Bode.txt'
data = pd.read_csv(file_path, sep='\t', encoding='latin1')

# Separar magnitud (dB) y fase (°) de la columna 'V(vo3)/V(Vin3)'
data['Magnitude (dB)'] = data['V(vo3)/V(Vin3)'].str.extract(r'\(([-+e\d\.]+)dB')[0].astype(float)
data['Phase (deg)'] = data['V(vo3)/V(Vin3)'].str.extract(r',([-+e\d\.]+)°\)')[0].astype(float)
