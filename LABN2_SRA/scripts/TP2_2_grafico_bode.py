...
file_path = 'LABN2_SRA/LTSpice_data/TP2_2_bode.txt'
data = pd.read_csv(file_path, sep='\t', encoding='latin1')

# Separar magnitud (dB) y fase (°) de la columna 'V(n022)'
data['Magnitude (dB)'] = data['V(n022)'].str.extract(r'\(([-+e\d\.]+)dB')[0].astype(float)
data['Phase (deg)'] = data['V(n022)'].str.extract(r',([-+e\d\.]+)°\)')[0].astype(float)
